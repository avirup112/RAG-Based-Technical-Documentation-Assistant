...
class MemoryManager:
    ...
    def list_sessions(self) -> List[Dict[str, Any]]:
        ...
        """
        sessions = (
            self.collection.find({}, {"session_id": 1, "chat_history": 1, "_id": 0})
            .sort("_id", -1)
            .limit(30)
        )
    ...
    def save_interaction(self, session_id: str, question: str, answer: str):
        ...
        if len(history) > 6:
            ...
            logger.info(
                f"Summarizing {len(messages_to_summarize)} old messages for session {session_id}"
            )
        ...
        self.collection.update_one(
            {"session_id": session_id}, {"$set": {"chat_history": history}}, upsert=True
        )
