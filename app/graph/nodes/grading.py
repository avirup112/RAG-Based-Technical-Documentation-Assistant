...
def grading_node(state: GraphState) -> GraphState:
    ...
    for doc in docs:
        ...
        if (
            compressed_text.upper() != "IRRELEVANT"
            and "IRRELEVANT" not in compressed_text[:20].upper()
        ):
            ...
            logger.debug(
                f"Compressed chunk from {original_len} to {new_len} chars ({(new_len/original_len)*100:.1f}%)"
            )
    ...
    logger.info(
        f"Contextual Compression complete. Kept {len(relevant_docs)} highly relevant snippets."
    )
        ...
        **state,
        ...
        "sender_metadata": {
            ...
            "signature": sign_payload(
                SENDER_GRADER, {"relevant_count": len(relevant_docs)}
            ),
        },
    }
