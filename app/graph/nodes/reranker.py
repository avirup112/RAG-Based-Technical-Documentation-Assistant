...
def reranker_node(state: GraphState) -> GraphState:
    ...
    if not documents:
        return {
            **state, 
            "reranked_docs": [],
            "sender_metadata": {"sender": SENDER_RERANKER, "signature": sign_payload(SENDER_RERANKER, {"docs_count": 0})}
        }
    ...
    # Sort and take top K
    reranked = sorted(docs_to_rerank, key=lambda x: x['rerank_score'], reverse=True)[:RERANK_K]
